...
def create_guest_test_client() -> Generator[TestClient, None, None]:
    ...
    client.app.state.fake_conversion_service = conversion_service
    client.app.state.fake_transfer_service = transfer_service
    client.app.state.fake_guest_token_cache = cache
    try:
        yield client
    finally:
        client.close()
...
def test_guest_verify_enqueues_with_guest_tier() -> None:
    with create_guest_test_client() as client:
        ...
        conversion_service = client.app.state.fake_conversion_service
...
def test_guest_get_job_returns_presigned_url_when_unencrypted() -> None:
    with create_guest_test_client() as client:
        ...
        client.app.dependency_overrides[get_encryption_service] = no_encryption
...
def test_guest_download_streams_decrypted_output_when_encryption_enabled() -> None:
    ...
    with create_guest_test_client() as client:
        ...
        client.app.dependency_overrides[get_minio_download_adapter] = lambda: FakeStreamingStorage(
            ciphertext
        )
        client.app.dependency_overrides[get_conversion_repository] = lambda: FakeJobRepository(
            _completed_job(output_file="output/guest/example.mov")
        )
        _job = client.post(
            "/api/guest/conversions/jobs",
            json={"source_format": "pdf", "target_format": "docx", "input_key": "k"},
...
def test_guest_events_streams_connected_and_terminal_event() -> None:
    with create_guest_test_client() as client:
        ...
        client.app.dependency_overrides[get_event_subscriber] = lambda: FakeSubscriber(
            {
                "job_id": "test-job-id",
                "status": "COMPLETED",
                "progress": "100",
