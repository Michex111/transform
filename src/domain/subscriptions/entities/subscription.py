...
class Subscription:
    ...
    def __post_init__(self) -> None:
        """Validates the subscription state."""
        if self.used_storage_bytes < 0:
            raise InvalidSubscriptionState("Used storage cannot be negative.")
    ...
    def release_storage(self, released_bytes: int) -> None:
        ...
        Args:
            released_bytes: Bytes to remove from usage.

        Raises:
            InvalidSubscriptionState: If released bytes are invalid.
        """
        if released_bytes < 0:
            raise InvalidSubscriptionState("released_bytes cannot be negative.")
        if released_bytes > self.used_storage_bytes:
            raise InvalidSubscriptionState(
                f"Cannot release {released_bytes} bytes from {self.used_storage_bytes} in use."
            )
