...
from src.application.ports.redis_port import RateLimiterPort
...
class ConversionAccessService:
    ...
    async def authorize_conversion(
        self,
        actor: ConversionActor,
        incoming_file_size_bytes: int,
        ...
        if incoming_file_size_bytes < 0:
            raise ValueError("incoming_file_size_bytes cannot be negative.")

        used_storage_bytes = await self._subscription_repository.get_used_storage_bytes(actor.actor_key)
        subscription = Subscription(tier=actor.tier, used_storage_bytes=used_storage_bytes)
        ...
        if actor.tier == SubscriptionTier.GUEST:
            ...
            allowed = await self._rate_limiter.allow(
                key=f"guest:conversion:{actor.ip_address}",
    ...
    async def commit_storage_usage(self, actor: ConversionActor, added_bytes: int) -> int:
        ...
        subscription = Subscription(tier=actor.tier, used_storage_bytes=current)
    ...
    async def _resolve_credit(
        ...
        credit = await self._credit_repository.get_credit(owner_id=user_id, period_key=period_key)
        if credit is not None:
            return credit
        return Credit.from_tier(owner_id=user_id, period_key=period_key, tier=tier)
