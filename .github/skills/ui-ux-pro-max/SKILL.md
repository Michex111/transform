...
| **Improve / optimize** | "Reduce React list rerenders" | Step 3 (explicit `react`, `ux`, or `web` domain) |
...
- Default icon library: **Phosphor (`@phosphor-icons/react`)**. `data/icons.csv` lists only common recommended icons, not the complete set.
- When a suitable icon is not in the recommendation table:
  - **Prefer choosing any more semantically fitting icon from Phosphor's full set**;
  - If Phosphor also has no ideal option, **Heroicons (`@heroicons/react`)** is the fallback — keep style consistent (line/fill, stroke weight, corner style).
...
| **Stroke Consistency** | Use a consistent stroke width within the same visual layer (e.g., 1.5px or 2px). | Mixing thick and thin stroke styles arbitrarily. | Inconsistent strokes reduce perceived polish. |
...
| **Icon Alignment** | Align icons to text baseline and maintain consistent padding. | Misaligned icons or inconsistent spacing around them. | Prevents subtle visual imbalance. |
